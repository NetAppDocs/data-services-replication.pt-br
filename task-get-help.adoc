...
[role="lead"]
A NetApp oferece suporte para o NetApp Console e seus serviços em nuvem de diversas maneiras. Opções extensas e gratuitas de autoatendimento estão disponíveis 24 horas por dia, 7 dias por semana, como artigos da base de conhecimento (KB) e um fórum da comunidade. Seu registro de suporte inclui suporte técnico remoto via sistema de tickets online.
...
Esse erro pode significar que a conta NSS e a empresa associada a ela não são a mesma empresa responsável pelo número de série da conta do NetApp Console (ou seja, 960xxxx) ou para o número de série do ambiente de trabalho. Você pode buscar assistência usando uma das seguintes opções:
